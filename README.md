...
Релизные APK собираются автоматически GitHub Actions ([.github/workflows/release.yml](.github/workflows/release.yml)) при пуше тега вида `v*` (например, `v1.0`) и публикуются вложением к соответствующему [GitHub Release](../../releases). APK подписан отладочным ключом Android SDK (production-ключ для публикации в Google Play пока не заведён), поэтому файл сразу пригоден для установки через `adb install` или sideloading, но не для загрузки в Play Console без перевыпуска подписи.
